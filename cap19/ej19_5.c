// Ejercicio 19.5. Conversión con iconv.
#include <errno.h>
#include <iconv.h>
#include <locale.h>
#include <stdio.h>
...
int main(void) {
   ...
   /* Buffers de trabajo */
   unsigned char latin1[64];
   memset(latin1, 0, sizeof(latin1));
   unsigned char utf8_back[64];
   memset(utf8_back, 0, sizeof(utf8_back));

   /* --- 1) UTF-8 -> ISO-8859-1 --- */
   iconv_t to_latin1 = iconv_open("ISO-8859-1", "UTF-8"); /* tocode, fromcode */
   if (to_latin1 == (iconv_t)-1) {
      perror("iconv_open to latin1");
      return 1;
   }
   ...
   *out = '\0'; /* terminador */
   ...
   if (to_utf8 == (iconv_t)-1) {
      perror("iconv_open to utf8");
      return 1;
   }

   in = (char *)latin1;
   inleft = strlen((char *)latin1);
   out = (char *)utf8_back;
   outleft = sizeof(utf8_back) - 1;
